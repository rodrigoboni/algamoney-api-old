    ...
    private Predicate[] getPredicates (LancamentoFilter filter, CriteriaBuilder builder, Root<Lancamento> from) {
        ...
        if (!StringUtils.isEmpty(filter.getDescricao())) {
            ...
            predicates.add(builder
                    .like(builder.lower(from.get("descricao")), "%" + filter.getDescricao().toLowerCase() + "%"));
        }

        if (filter.getDataVencimentoDe() != null) {
            predicates.add(builder.greaterThanOrEqualTo(from.get("dataVencimento"), filter.getDataVencimentoDe()));
        }

        if (filter.getDataVencimentoAte() != null) {
            predicates.add(builder.lessThanOrEqualTo(from.get("dataVencimento"), filter.getDataVencimentoDe()));
        }
